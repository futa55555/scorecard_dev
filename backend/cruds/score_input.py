...
def get_inning(
    ...
    inning = (
        ...
        .filter(models.Inning.inning_id == inning_id)
        .first()
    )
    if not inning:
...
def get_all_innings_with_events(
    ...
    all_innings_with_events = (
        ...
        .order_by(models.Inning.inning_id)
        .all()
    )
...
def get_pitch_events_of_latest_atbat(
    ...
    pitch_events_of_latest_atbat = (
        ...
        .filter(models.PitchEvent.atbat_id == latest_atbat.atbat_id)
...
def update_inning_score(
    ...
    inning = db.query(models.Inning).filter(models.Inning.inning_id == inning_id).first()
    if inning:
        inning.score = score
        db.commit()
        db.refresh(inning)
