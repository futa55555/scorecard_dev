...
def get_inning_score(
    ...
    top_team_innings = [
        i for i in innings if i.top_bottom == models.TopBottomEnum.top]
    total_score = calc_total_score(top_team_innings)

    top_team = schema.ForInningScore(
        team_short_name=game.top_team.short_name,
        team_inning_id=[i.inning_id for i in top_team_innings],
        team_inning_score=[i.score for i in top_team_innings],
        team_total_score=total_score["team_total_score"],
        team_total_hit=total_score["team_total_hit"]
    )

    bottom_team_innings = [
        i for i in innings if i.top_bottom == models.TopBottomEnum.bottom]
    total_score = calc_total_score(bottom_team_innings)

    bottom_team = schema.ForInningScore(
        team_short_name=game.bottom_team.short_name,
        team_inning_id=[i.inning_id for i in bottom_team_innings],
        team_inning_score=[i.score for i in bottom_team_innings],
        team_total_score=total_score["team_total_score"],
        team_total_hit=total_score["team_total_hit"]
    )
...
def get_member_position_type(
    ...
    return models.PositionTypeEnum.NOT
...
def get_bench_member(
    ...
    top_team_member_with_position_type = [
        schema.MemberWithPositionType(
            member_id=sm.game_member_id,
            member_name=sm.person.name,
            member_position_type=get_member_position_type(
                db, sm.person_id, game.date)
...
def get_entry_history(
    ...
    top_team_entry_state = [
        schema.EntryState(
            ...
            game_member_id=sm.game_member_id,
...
def get_battery(
    ...
    top_team_pitcher = [
        ...
        if es.position == models.PositionEnum.P
    ]
    ...
    top_team_catcher = [
        ...
        for es in entry_history.top_team.entry_state
        if es.position == models.PositionEnum.C
    ]
...
def get_extrabase_hit(
    ...
    top_team_two_base_hit = [
        schema.MemberWithName(
            member_id=a.responsible_batter.game_member_id,
...
def aggregate_advance_events(
    ...
    """
    advance_events_of_latest_inning = crud.get_advance_events_of_latest_inning(
        db, game_id)
    ...
    for advance_event in advance_events_of_latest_inning:
        ...
        if advance_event.from_base not in (0, 1, 2, 3):
            raise ValueError(
                f"from_base ({advance_event.from_base}) is invalid.")
        ...
        if advance_event.from_base == 0:
            ...
            if advance_event.runner_id == batter_id:
                ...
                raise ValueError(
                    f"mismatch in runner (#{runner.person.name}) and batter (#{batter.person.name})")
        else:
            if advance_event.runner_id == runners_id[advance_event.from_base]:
                pass
            else:
                raise ValueError(
                    f"mismatch in runner (#{runner.person.name}) and previous runner (#{crud.get_game_member_by_id(db, runners_id[advance_event.from_base])})")
        ...
        if advance_event.is_out:
            ...
            if advance_event.to_base in (1, 2, 3):
                if runners_id[advance_event.to_base] == 0:
                    ...
                    raise ValueError(
                        f"runner (#{crud.get_game_member_by_id(db, runners_id[advance_event.to_base])}) is already on the base {advance_event.to_base}")
